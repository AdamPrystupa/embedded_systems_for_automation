...
#include "commands.h"    // żeby móc wywołać process_command()
#include "leds.h"        // jeśli chcesz w przerwaniu mrugać LEDem
...
void on_uart_rx() {
    ...
    while (uart_is_readable(UART_ID)) {
        led_toggle(); // prosta sygnalizacja, że otrzymano znak
        uint8_t ch = uart_getc(UART_ID);
        if (ch == '\r') {
            process_command();
            // process_command wewnątrz czyści term_buf i term_idx
        } else {
            if (term_idx < N_TERM_BUF - 1) {
                term_buf[term_idx++] = ch;
                term_buf[term_idx] = '\0';
            } else {
                // Bufor pełny – można np. nadpisać, zasygnalizować błąd itd.
            }
        }
    }
}
